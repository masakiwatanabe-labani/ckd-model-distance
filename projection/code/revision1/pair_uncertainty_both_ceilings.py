    ...
    for _, pr in P.iterrows():
        ...
        for i in range(N_BOOT):
            ...
            if np.isfinite(ra) and np.isfinite(rb):
                ...
                sa = 2 * ra / (1 + ra) if ra > 0 else 0.0
                sb = 2 * rb / (1 + rb) if rb > 0 else 0.0
                ceil_sb[i] = np.sqrt(min(max(sa, 0.0), 1.0) * min(max(sb, 0.0), 1.0))
            else:
                n_undef += 1
        gap = ceil - cosb
        gap_sb = ceil_sb - cosb
        ok = np.isfinite(gap)
        ok_sb = np.isfinite(gap_sb)
        ...
        rows.append(dict(
            ...
            gap_sb_median=float(np.nanmedian(gap_sb)) if ok_sb.any() else np.nan,
            gap_sb_lo=float(np.nanpercentile(gap_sb, 2.5)) if ok_sb.any() else np.nan,
            gap_sb_hi=float(np.nanpercentile(gap_sb, 97.5)) if ok_sb.any() else np.nan,
            p_gap_sb_le0=float(np.mean(gap_sb[ok_sb] <= 0)) if ok_sb.any() else np.nan))
    T = pd.DataFrame(rows)
    T.round(4).to_csv(OUT / "pair_uncertainty_both_ceilings.tsv", sep="\t", index=False)
